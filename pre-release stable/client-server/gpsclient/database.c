...
dbctx_t *db_connect(void)
{
	...
	snprintf(conn_str, sizeof(conn_str),
		 "host=%s port=%i dbname=%s user=%s password=%s connect_timeout=10",
	      ...
	      struct db_config *cfg)
{
	...
	snprintf(tcfg.server_host, sizeof(tcfg.server_host), "%s", PQgetvalue(result, 0, 7));
...
void db_debugcfg(const struct db_config *cfg)
{
	...
	debug(DEBUG_INFO, "server-host=%s server-port=%i", cfg->server_host, cfg->server_ctlport);
